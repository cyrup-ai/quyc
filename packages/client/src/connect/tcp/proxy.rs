...
//!
//! Support for HTTP CONNECT tunnels and SOCKS4/5 proxy protocols
//! with comprehensive handshake implementations.
...
///
/// Creates an HTTP CONNECT tunnel to the target URI through a proxy server.
/// Supports optional proxy authentication with Basic auth.
///
/// # Arguments
/// * `proxy_stream` - Connected stream to the proxy server
/// * `target_uri` - The target URI to tunnel to
/// * `auth` - Optional Basic auth credentials (base64 encoded)
...
/// SOCKS version enumeration for proxy protocol selection.
...
///
/// Dispatches to the appropriate SOCKS version handler based on the version parameter.
///
/// # Arguments
/// * `stream` - Connected stream to the SOCKS proxy
...
///
/// Implements SOCKS4 and SOCKS4A protocols with hostname support.
/// Uses 0.0.0.x IP format to indicate hostname follows for SOCKS4A.
...
///
/// Implements full SOCKS5 protocol with authentication negotiation
/// and support for IPv4, IPv6, and domain name addressing.
