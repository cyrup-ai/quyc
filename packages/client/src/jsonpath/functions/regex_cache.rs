...
pub static REGEX_CACHE: std::sync::LazyLock<RegexCache> = std::sync::LazyLock::new(RegexCache::new);
    ...
    F: FnOnce() -> bool + Send + 'static,
{
    ...
    match rx.recv_timeout(timeout_duration) {
        ...
        Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
            let elapsed = start_time.elapsed();
            log::warn!(
                "Regex execution timed out after {elapsed:?} - potential ReDoS attack"
            );
